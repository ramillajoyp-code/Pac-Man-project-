...
class HUD:
    ...
    def draw(self, screen, score, high_score, lives, level, state, state_timer=0, pac_color=(255, 255, 0)):
        ...
        if state != "START_MENU":
            ...
            for i in range(lives):
                ...
                ly = 36 * TILE_SIZE + 15
                pygame.draw.circle(screen, pac_color, (int(lx), int(ly)), TILE_SIZE // 2)
        ...
        if state == "START_MENU":
            ...
            title_text = self.large_font.render("TOM AND JERRY", True, (255, 255, 0))
            title_x = SCREEN_WIDTH // 2 - title_text.get_width() // 2
            title_y = border_margin + 30
            screen.blit(title_text, (title_x, title_y))
            
            # Interactive subtitle
            sub_text = self.font.render("PACMAN EDITION", True, (255, 255, 0))
            ...
            # Pause title
            p_text = self.large_font.render("PAUSED", True, (255, 255, 0))
            screen.blit(p_text, (SCREEN_WIDTH // 2 - p_text.get_width() // 2, 250))

            # Restart option
            restart_text = self.medium_font.render("R - RESTART", True, (255, 255, 255))
            screen.blit(restart_text, (SCREEN_WIDTH // 2 - restart_text.get_width() // 2, 320))

            # Quit option
            quit_text = self.medium_font.render("Q - QUIT", True, (255, 255, 255))
            screen.blit(quit_text, (SCREEN_WIDTH // 2 - quit_text.get_width() // 2, 370))

            # Resume option
            resume_text = self.small_font.render("P - RESUME", True, (0, 255, 0))
            screen.blit(resume_text, (SCREEN_WIDTH // 2 - resume_text.get_width() // 2, 430))